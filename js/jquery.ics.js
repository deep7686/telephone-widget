// ICS-Phonenumber widget
// author: Deepak Sharma - http://www.iqbsys.com
// Date: 03 July, 2013
// Version: 1.0
// Revision: 16
// web: www.iqbsys.com
...
//alert(id);
		...
		case "dropdown":
		case "select":
			data = $(id).icsCountry(settings).data("dd");
			break;
		}
		return data;
	}
...
$.icsCountry = {};
$.icsCountry = {};
$.extend(true, $.icsCountry, msBeautify);
$.extend(true, $.icsCountry, msBeautify);
...
function dd(element, settings) {
	...
	var _childHeight = function (val) {
		...
		if (val) {
			if (val == -1) { //auto
				...
				$("#"+childid).css("height", "133px");// This is the height of your country list drop down  you can change it
			};
			return false;
		};
		//else return height
	...
	/*********************** </layout> *************************************/
	var on_change = function () { // Here you can add any new method whic you can call on country change
		...
		setHiddenValueAndPhoneValue(this.name,this.value,this.getAttribute("data-format"));	
	...
	var _getDataAndUI = function () {
		...
		if (ele.selectedIndex == -1) {
			...
			if (ui.length > 1) {
				...
				//alert(option.title);
				//alert(option.getAttribute("data-format"));
				setMsak(option.value,"phone_number",option.getAttribute("data-format"));
				//You can add your method which you can call on widget load.
...
function setHiddenValueAndPhoneValue(slelectValue,C_value,dataFormat)// I have call this method on change and on widget load.
{ 
	
	setMsak(C_value,"phone_number",dataFormat);
}
...
$.fn.icsCountry = $.fn.icsCountry; //make a copy
})(jQuery);